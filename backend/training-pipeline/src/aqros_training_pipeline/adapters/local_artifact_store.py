...
(Requirement 13.4 — swapping for S3 touches only this file). Path layout
``{base_dir}/{model_name}/v{model_version}/model.joblib`` deterministically
encodes both the (composite) model name and version (Requirement 13.1).
...
(Requirement 13.2) — the check-then-write is atomic within that thread to
avoid a TOCTOU race between concurrent writers for the same version.
Serialization is uniform ``joblib`` bytes for all four Model_Types
(Key Design Decision 1); this store treats the payload as opaque bytes.
"""
...
from aqros_training_pipeline.domain.ports import ArtifactAlreadyExistsError, ArtifactStore
